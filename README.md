...
| 参数范围 | ε ∈ {0, 0.001, 0.01, 0.05} | p ∈ {0, 0.01, 0.1, 0.3} |

### 5. 评估指标

项目要求从准确率 (Accuracy)、算法复杂度 (Algorithm Complexity)、峰值信噪比 (PSNR) 三个维度评估系统性能。每次实验结束后自动输出完整的评估报告。

#### 5.1 准确率 (Accuracy)

衡量信道编码的纠错能力——传输过程中有多少 bit 错误被成功纠正。

| 指标 | 计算方法 | 说明 |
|---|---|---|
| 信道原始误码率 | 信道翻转/删除的 bit 数 ÷ 编码后总 bit 数 | 无纠错时的原始错误水平 |
| Viterbi 纠错后 BER | 译码后残留错误 ÷ 原始 bit 数 | 卷积码纠错后的效果 |
| 纠错成功率 | (1 - 纠错后 BER ÷ 原始误码率) × 100% | 越高说明信道编码越有效 |
| 源编码压缩率 | 原始图像 bit 数 ÷ 压缩后 bit 数 | 大于 1 表示有压缩，越高越好 |

#### 5.2 算法复杂度 (Algorithm Complexity)

分别测量各环节耗时，反映算法的计算开销。

| 环节 | 算法 | 大 O 复杂度 | 输出中显示 |
|---|---|---|---|
| 源编码 | DCT + 量化 + RLE + Huffman | O(N) N=像素数 | 编码耗时 (s) |
| 信道编码 | (2,1,7) 卷积码 | O(N) N=源 bit 数 | 编码耗时 (s) |
| 交织 | 块交织 (64×128) | O(N) | 交织耗时 (s) |
| 解交织+信道译码 | Viterbi 算法 | O(N·2ᴷ) K=7 约束长度 | 解交织+译码耗时 (s) |
| 源解码 | Huffman + IDCT | O(N) | 解码耗时 (s) |
| 端到端总耗时 | 以上全部之和 | — | 总耗时 (s) |

> 图像分辨率也在输出中显示，用于归一化复杂度对比。
